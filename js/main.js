...
d3.csv("https://github.com/NurNazira/NurNazira.github.io/blob/master/Malaysia%20unemployment%20people%20by%20state.csv", function(csv)
  root = csv;
  root.x0 = h / 2;
  root.y0 = 0;
...
function tabfilter(d) {
  var filterString = "";
  if (d.depth==0){
     setParameter('filterInput','State/Country');
  }else if(d.depth==1){
    filterString = 'Year|'+d.name;
     setParameter('filterInput',filterString);
  } else {
     filterString = 'Unemployed ('000)|'+d.parent.name+'|'+d.name;
     setParameter('filterInput',filterString);
  }
} 
